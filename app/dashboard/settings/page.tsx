import { User, BellRing, ShieldCheck, Key, Buildings } from '@phosphor-icons/react/dist/ssr'
        ...
        <div className="md:col-span-9 space-y-6">
          ...
          <div className="bg-white rounded-[2rem] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] border border-zinc-100 overflow-hidden">
            <div className="px-8 py-6 border-b border-zinc-100">
              <h2 className="text-lg font-semibold text-zinc-950">Monitored Organizations</h2>
              <p className="text-sm text-zinc-500">Domains and assets to track in the threat feed.</p>
            </div>
            
            <div className="p-8 border-b border-zinc-100">
              {organizations.length === 0 ? (
                <p className="text-sm text-zinc-500">No organizations configured. Add one below.</p>
              ) : (
                <ul className="space-y-3">
                  {organizations.map(org => (
                    <li key={org.id} className="flex justify-between items-center p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
                      <div>
            ...
            <form action={addOrganization} className="p-8 space-y-6">
              <h3 className="text-sm font-semibold text-zinc-900">Add New Organization</h3>
              <div className="flex flex-col gap-4 sm:flex-row">
                ...
                <div className="flex-1 flex flex-col gap-2">
                  <label className="text-xs font-medium text-zinc-700">Primary Domain</label>
                  <input 
                    name="domain"
